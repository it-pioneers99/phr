...
function show_annual_leave_dialog(data) {
    ...
    const dialog = new frappe.ui.Dialog({
        ...
        fields: [
            ...
            {
                label: __('Is Additional Annual Leave'),
                fieldtype: 'Check',
                default: employeeInfo.is_additional_annual_leave || false,
                read_only: 1
            },
            {
            ...
            {
                label: __('Calculation Rate'),
                fieldtype: 'Data',
                default: annualLeave.days_per_month ? 
                    `${annualLeave.days_per_month} days/month${annualLeave.is_additional_annual_leave ? ' ⭐' : ''}` : 
                    'Not calculated',
                read_only: 1
            },
            {
                label: __('Calculation Reason'),
                fieldtype: 'Data',
                default: annualLeave.calculation_reason || 'Standard calculation',
                read_only: 1
            },
            {
...
function calculate_contract_dates(frm) {
    ...
    frm.set_value('notification_sent_90_days', 0);
    frm.set_value('notification_sent_30_days', 0);
    frm.set_value('last_notification_date', null);
...
function send_contract_notification(frm) {
    if (!frm.doc.contract_end_date) {
        frappe.msgprint(__('Please calculate contract dates first'));
        return;
    }
    
    frappe.call({
        method: 'phr.phr.phr.utils.contract_notifications.check_contract_expiration_notifications',
        callback: function(r) {
            if (r.message && r.message.status === 'success') {
                frappe.msgprint(__('Contract notifications sent successfully: ' + r.message.message));
            } else {
                frappe.msgprint(__('Error sending notifications: ' + (r.message.message || 'Unknown error')));
            }
        }
    });
}
...
function reset_contract_notifications(frm) {
    ...
    frappe.call({
        method: 'phr.phr.phr.utils.contract_notifications.reset_notification_flags',
        args: {
...
// Update the existing add_phr_buttons function to include contract buttons
function add_contract_buttons(frm) {
    // Contract Management Buttons
    frm.add_custom_button(__('Calculate Contract Dates'), function() {
        calculate_contract_dates(frm);
    }, __('Contract'));
    
    frm.add_custom_button(__('Send Contract Notifications'), function() {
        send_contract_notification(frm);
    }, __('Contract'));
    
    frm.add_custom_button(__('Reset Notifications'), function() {
        reset_contract_notifications(frm);
    }, __('Contract'));
}

// Update the main add_phr_buttons function
function add_phr_buttons_updated(frm) {
    // Main PHR Section
    frm.add_custom_button(__('PHR Management'), function() {
        show_phr_main_dialog(frm);
    }, __('PHR'));
...
// Update the refresh function to use the new button function
frappe.ui.form.on('Employee', {
    refresh: function(frm) {
        // Add custom buttons for PHR functionality
        add_phr_buttons_updated(frm);
        ...
        // Auto-calculate contract dates when joining date changes
